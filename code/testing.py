...
if __name__ == "__main__":
    ...
    output_full = 'full_result_scenes.csv'
    output_summary = 'summary_result_scenes.csv'
    param_file_header = 'tuning summary/tuning_summary_'
    true_att = 0
    n = 1000
    ...
    results = {'Bias': [], 'Cover': [], 'Length': [], 'Panel': [], 'Heterogeneous': [], 'Unobserved': [], 'Method': [], 'Model': []}

    for data_setting in data_settings:
        panel, hetero, unobs = data_setting
        dct_best_param = parse_parameter(param_file_header + str(int(panel)) + str(int(hetero)) + str(int(unobs)) + '.csv')
        ...
        for _ in tqdm(range(n)):
            ...
            for model_setting in model_settings:
                ...
                try:
                    ...
                    results.get('Heterogeneous').append(hetero)
                    results.get('Unobserved').append(unobs)
    ...
    # get means
    df_summary = df_result.groupby(['Panel', 'Heterogeneous', 'Unobserved', 'Method', 'Model'])[['Bias', 'Cover', 'Length']].mean()
    df_median = df_result.groupby(['Panel', 'Heterogeneous', 'Unobserved', 'Method', 'Model'])['Bias'].median().to_frame('Median')
    # RMSE
    df_rmse = df_result.groupby(['Panel', 'Heterogeneous', 'Unobserved', 'Method', 'Model'])['Bias'].apply(list).\
